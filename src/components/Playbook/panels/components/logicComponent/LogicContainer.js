...
<div class='scroller-content logic-scroller-content'>  
  ...
  <div class='buttons'>   
    <% if (count < 5) { %>    
      <button class='elif'>Add Else If</button>   
      <% if (noElse) { %>    
        <button class='else'>Add Else</button>    
      <% } %>    
    <% } %>  
  </div>
</div>
...
export default Backbone.View.extend(_.extend({}, scrollable, {
  ...
  closeSelect: function () {
    ...
    //   var t = this,
    //     e = this.$el.position(),
    //     i = this.fullClose ? -this.selectView.$el.outerWidth() : e.left + this.$el.outerWidth() - this.selectView.$el.outerWidth()
    //   this.selectView.animatePosition(i, function () {
    //     t.disableBodyClick(), t.selectView && t.selectView.remove(), t.selectView = null, t.setActiveField(null)
    //   })
    // }
    if (this.selectView) {
      const that = this
