...
export default joint.mvc.View.extend({
  options: {
    paper: void 0,
    graph: void 0,
  ...
  init: function () {
    ...
    var t = this.collection = this.options.collection || this.collection || new Backbone.Collection()
    if (!t.comparator) {
  ...
  addHandle: function (t) {
    ...
    e.html(t.content || '')
    joint.util.setAttributesBySelector(e, t.attrs)
    this.$selectionWrapper.append(e)
    _.each(t.events, function (e, i) {
      joint.util.isString(e)
  ...
  stopSelecting: function (t) {
    switch (this._action) {
      case 'selecting':
        ...
        var a = joint.V(o.viewport).toLocalPoint(e.left, e.top)
        ...
        i /= r.sx
        s /= r.sy
        var c = joint.g.rect(a.x, a.y, i, s)
        var u = this.getElementsInSelectedArea(c)
        var d = this.options.filter

        if (joint.util.isArray(d)) {
          u = joint.util.reject(u, function (t) {
            return joint.util.contains(d, t.model) || joint.util.contains(d, t.model.get('type'))
          })
        } else {
          joint.util.isFunction(d) && (u = joint.util.reject(u, function (t) {
            return d(t.model)
          }))
        }
        this.model.reset(joint.util.pluck(u, 'model'), {
          ui: true,
        })
        break
      case 'translating':
  ...
  getElementsInSelectedArea: function (t) {
    ...
    if (this.options.useModelGeometry) {
      ...
      return joint.util.filter(joint.util.map(s, e.findViewByModel, e))
    }
  ...
  startRotating: function (t) {
    ...
    var s = joint.util.transform(this.model.toArray(), function (t, e) {
      t[e.id] = joint.g.normalizeAngle(e.get('angle') || 0)
    })
    this._rotation = {
      center: e,
      clientAngle: joint.g.point(i).theta(e),
      initialAngles: s,
    }
  },
  startResizing: function (t) {
    ...
    var a = i.getBBox(o)
    var r = joint.util.invoke(o, 'getBBox')
    var l = joint.util.min(r, 'width').width
    var c = joint.util.min(r, 'height').height
    this._resize = {
  ...
  doRotate: function (t) {
    ...
    var s = e.clientAngle - joint.g.point(n).theta(e.center)
    if (Math.abs(s) > 0.001) {
      this.model.each(function (t) {
        var n = joint.g.snapToGrid(e.initialAngles[t.id] + s, i)
